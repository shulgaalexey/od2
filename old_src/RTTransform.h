...
void LogSignal2File( const char * const pFileName, const _SourceT& aSrc )
{
	HANDLE hFile = ::CreateFile( pFileName, GENERIC_READ | GENERIC_WRITE, 0, 0, CREATE_NEW, 0, 0 );
	MySourceData::const_iterator mdIterator = aSrc.begin();
	for( ; mdIterator != aSrc.end(); ++mdIterator )
	{
		char str[ 7 ];
		sprintf_s( str, "%d\n", *mdIterator );
		DWORD dw;
...
private:
	...
	void CalcSplineParams( const double &dYA, const double &dYB, const double &dYADif, const double &dYBDif,
		double &dYDif2, double &dD ) const
	{
...
public:
	...
	{
		_SourceT aDst;
		for_each( aSrc.begin() + 1, aSrc.end(), SpeedDownSignal<typename _SourceT::value_type, typename _SourceT>( m_nAFD, aDst ) );
		return aDst;
	}
};
//-------------------------------------------------------------------------------------------------
